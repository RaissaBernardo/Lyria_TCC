...
import { FaEye, FaEyeSlash, FaChalkboardTeacher, FaBriefcase, FaUserFriends, FaVolumeUp } from "react-icons/fa";
...
const personaDetails = {
  professor: {
    ...
    title: "Professora",
    description: "Didática, acessível e objetiva. Ideal para aprender novos conceitos e tirar dúvidas complexas.",
  },
  empresarial: {
    icon: <FaBriefcase />,
    title: "Assistente Corporativa",
    description: "Profissional, direta e focada em resultados. Perfeita para o ambiente de trabalho e tarefas objetivas.",
  },
  social: {
    icon: <FaUserFriends />,
    title: "Companheira Social",
    description: "Acolhedora, empática e compreensiva. Uma parceira para conversas do dia a dia e suporte emocional.",
  },
};
...
function LoginRegisterPage() {
  ...
  const renderRegisterStep1 = () => (
    <>
      ...
      <p className="form-subtitle">Junte-se a nós e explore o universo LyrIA.</p>
      ...
      <div className="input-group">
        <input type={passwordVisible ? "text" : "password"} placeholder="Senha" required value={senha} onChange={(e) => setSenha(e.target.value)} />
        <span className="password-toggle-icon" onClick={() => setPasswordVisible(!passwordVisible)}>
          {passwordVisible ? <FaEyeSlash /> : <FaEye />}
        </span>
      </div>
      <div className="input-group">
        <input type={confirmPasswordVisible ? "text" : "password"} placeholder="Confirmar Senha" required value={confirmarSenha} onChange={(e) => setConfirmarSenha(e.target.value)} />
        <span className="password-toggle-icon" onClick={() => setConfirmPasswordVisible(!confirmPasswordVisible)}>
          {confirmPasswordVisible ? <FaEyeSlash /> : <FaEye />}
        </span>
      </div>
      <button type="submit" className="submit-btn" disabled={loading}>PRÓXIMO</button>
    </>
  );
  ...
  const renderRegisterStep2 = () => (
    <>
      ...
      <p className="form-subtitle">Escolha a personalidade e a voz que mais combinam com você.</p>
      
      <div className="selection-group">
        <label>Escolha uma Persona:</label>
        <div className="persona-options">
            {Object.keys(personas).map(key => (
                <div key={key} className={`persona-card ${selectedPersona === key ? 'selected' : ''}`} onClick={() => setSelectedPersona(key)}>
      ...
      <div className="selection-group">
        ...
        <div className="voice-selection-container">
            <select id="voice-select" className="voice-select-input" value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)}>
                {availableVoices.map(voice => (
                    <option key={voice.value} value={voice.value}>{voice.label}</option>
                ))}
            </select>
            <button type="button" className="test-voice-btn" onClick={handleTestVoice}><FaVolumeUp /></button>
        </div>
      </div>
      
      <div className="button-group">
        <button type="button" className="secondary-btn" onClick={() => setStep(1)}>VOLTAR</button>
        <button type="submit" className="submit-btn" disabled={loading}>{loading ? "CADASTRANDO..." : "CADASTRAR"}</button>
      </div>
    </>
  );
  ...
  return (
    <div className="auth-body">
      <div className={`form-container ${isLogin ? "login-active" : "register-active"}`}>
        <form onSubmit={handleAuth} className={`form-content ${animationClass}`}>
          {isLogin ? renderLogin() : (step === 1 ? renderRegisterStep1() : renderRegisterStep2())}
        </form>
